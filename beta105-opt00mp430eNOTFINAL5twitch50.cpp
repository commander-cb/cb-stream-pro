...
// Capture the screen and fill the buffer with BGRA pixels.
...
bool initializeVideoEncoder(AVFormatContext* fmtCtx, AVCodecContext** codecContext, int width, int height) {
    ...
    *codecContext = avcodec_alloc_context3(codec);
    ...
    (*codecContext)->gop_size = 60;  // Keyframe every 2 sec at 30 FPS.
    (*codecContext)->max_b_frames = 0;
    (*codecContext)->flags |= AV_CODEC_FLAG_GLOBAL_HEADER;
      
    AVDictionary* opts = nullptr;
    av_dict_set(&opts, "preset", "p2", 0);
    av_dict_set(&opts, "bitrate", "1000000", 0);  // 1 Mbps for testing.
...
void streamingSession(int width, int height) {
    while (!stopProgram) {
        // Reset isRecording and any necessary timestamps here.
        isRecording = true;
        std::cout << "Starting streaming session. Press U to stop.\n";
        recordingThread(width, height);  // If this returns (due to error/disconnect), we reconnect.
        isRecording = false;
...
int main() {
    ...
    // Start the streaming session in a reconnection loop.
