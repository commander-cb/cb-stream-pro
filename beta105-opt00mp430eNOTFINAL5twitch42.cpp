...
bool initializeVideoEncoder(AVFormatContext* fmtCtx, AVCodecContext** codecContext, int width, int height) {
    ...
    *codecContext = avcodec_alloc_context3(codec);
    ...
    av_dict_set(&opts, "bitrate", "1500000", 0);
    av_dict_set(&opts, "maxrate", "2200000", 0);
...
void streamingSession(int width, int height) {
    while (!stopProgram) {
        isRecording = true;
        std::cout << "Starting streaming session. Press U to stop.\n";
        recordingThread(width, height);  // If this returns due to error, reconnect.
        isRecording = false;
        std::cerr << "Streaming session ended unexpectedly. Reconnecting in 5 seconds...\n";
...
void streamingSession(int width, int height) {
    ...
    while (!stopProgram) {
        ...
        // Run your streaming session.
        // When recordingThread returns, assume an error or disconnect occurred.
...
void audioCaptureThread(AVFormatContext* fmtCtx, AVCodecContext* audioCodecCtx) {
    ...
    // Here, we set a 3-second buffer duration (in 100-nanosecond units).
    REFERENCE_TIME hnsBufferDuration = 30000000; // 3 seconds
    ...
    // Prepare an AVPacket for encoded audio packets.
...
// When we have enough samples (1024 per channel), send an audio frame.
// When we have enough samples (at least 1024 per channel), send audio frames.
while (audioAcc[0].size() >= 1024) {
    // Allocate an AVFrame for audio with exactly 1024 samples.
    ...
    // For each channel, copy exactly 1024 samples from the accumulator.
    for (int ch = 0; ch < numChannels; ch++) {
        if (audioAcc[ch].size() < 1024) {
            std::cerr << "Unexpected: channel " << ch << " accumulator has fewer than 1024 samples.\n";
            continue;
        }
        memcpy(frame->data[ch], audioAcc[ch].data(), 1024 * sizeof(float));
        // Remove the 1024 samples we've just copied.
        audioAcc[ch].erase(audioAcc[ch].begin(), audioAcc[ch].begin() + 1024);
    }
    
    // Set the frame's PTS and update the global counter.
    frame->pts = audioPts;
    audioPts += 1024;
    
    // Log for debugging.
    std::cout << "Sending audio frame: nb_samples = " << frame->nb_samples 
...
av_log_set_level(AV_LOG_DEBUG);
    ...
    // Option 1: Use a loop directly in main.
    while (!stopProgram) {
        streamingSession(width, height);
    }
    
    // Option 2: Or run streamingSession in a dedicated thread (if you prefer to keep your keyboard hook running).
    /*
    std::thread streamer([&]() { streamingSession(width, height); });
