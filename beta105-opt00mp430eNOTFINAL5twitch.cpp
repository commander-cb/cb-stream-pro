...
bool initializeFFmpeg(AVFormatContext** formatContext, AVCodecContext** codecContext, int width, int height, const char* rtmpUrl) {
    // Allocate format context for FLV (Twitch requires FLV)
    avformat_alloc_output_context2(formatContext, nullptr, "flv", rtmpUrl);
    if (!*formatContext) {
        std::cerr << "Error: Could not allocate FLV format context.\n";
        return false;
    }

    // Select NVIDIA NVENC Encoder (or change to another encoder if needed)
    ...
    *codecContext = avcodec_alloc_context3(codec);
    ...
    // Set time base and frame rate. (Adjust frame rate if necessary)
    (*codecContext)->time_base = (AVRational){1, 1000000};  // Microsecond resolution
    (*codecContext)->framerate = (AVRational){30, 1};         // 30 FPS (update if needed)
    (*codecContext)->pkt_timebase = (AVRational){1, 1000000};
    (*codecContext)->pix_fmt = AV_PIX_FMT_YUV420P;
    (*codecContext)->gop_size = 30;  // Keyframe every second at 30 FPS
    (*codecContext)->max_b_frames = 0;

    // NVENC requires global header flag
    (*codecContext)->flags |= AV_CODEC_FLAG_GLOBAL_HEADER; 

    // Configure encoder options (adjust bitrate to Twitch limits; typical max is ~6000 kbps for 1080p)
    AVDictionary* opts = nullptr;
    av_dict_set(&opts, "preset", "p4", 0);
    av_dict_set(&opts, "tune", "hq", 0);
    av_dict_set(&opts, "bitrate", "6000000", 0);   // Example: 6 Mbps target bitrate
    av_dict_set(&opts, "maxrate", "6000000", 0);
    av_dict_set(&opts, "bufsize", "12000000", 0);
