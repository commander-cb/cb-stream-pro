...
while (audioAcc[0].size() >= 1024) {
    ...
    // Copy exactly 1024 samples per channel from the accumulator into the frame.
    for (int ch = 0; ch < numChannels; ch++) {
        memcpy(frame->data[ch], audioAcc[ch].data(), 1024 * sizeof(float));
        // Erase the first 1024 samples for this channel.
        audioAcc[ch].erase(audioAcc[ch].begin(), audioAcc[ch].begin() + 1024);
    }
    
    // Set the frame's PTS and update the PTS counter.
    frame->pts = audioPts;
    audioPts += 1024;
