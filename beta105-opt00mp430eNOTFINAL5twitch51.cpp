...
struct CapturedFrame {
    ...
    int64_t timestamp;         // microseconds (relative to session start)
};
...
bool initializeVideoEncoder(AVFormatContext* fmtCtx, AVCodecContext** codecContext, int width, int height) {
    ...
    *codecContext = avcodec_alloc_context3(codec);
    ...
    (*codecContext)->gop_size = 60;  // Keyframe every 2 seconds at 30 FPS.
    (*codecContext)->max_b_frames = 0;
    (*codecContext)->flags |= AV_CODEC_FLAG_GLOBAL_HEADER;
      
    AVDictionary* opts = nullptr;
    av_dict_set(&opts, "preset", "p2", 0);
    av_dict_set(&opts, "bitrate", "2000000", 0);  // ? Mbps
...
void audioCaptureThread(AVFormatContext* fmtCtx, AVCodecContext* audioCodecCtx) {
    ...
    REFERENCE_TIME hnsBufferDuration = 60000000; // 6 seconds
    ...
    while(isRecording) {
         ...
         while(audioAcc[0].size() >= 1024) {
              ...
              if (avcodec_send_frame(audioCodecCtx, frame) < 0) {
                  ...
                  int ret;
                  while ((ret = avcodec_receive_packet(audioCodecCtx, packet)) >= 0) {
                      ...
                      if(ret < 0) {
                           std::cerr << "Error writing audio packet: " << av_err2str(ret) << "\n";
                           isRecording = false;
                           break;
                      }
                  ...
                  if(ret != AVERROR(EAGAIN) && ret != AVERROR_EOF) {
                      std::cerr << "Audio encoding error: " << av_err2str(ret) << "\n";
                  }
              }
              av_frame_free(&frame);
         }
    ...
    std::thread encodingThread([&]() {
        while (isRecording || !frameBuffer.isEmpty()) {
            ...
            if (avcodec_send_frame(videoCodecCtx, frame) >= 0) {
                int ret;
                while ((ret = avcodec_receive_packet(videoCodecCtx, packet)) >= 0) {
                    av_packet_rescale_ts(packet, videoCodecCtx->time_base, formatContext->streams[0]->time_base);
                    packet->stream_index = 0;
                    ret = av_interleaved_write_frame(formatContext, packet);
                    if (ret < 0) {
                        std::cerr << "Error writing video packet: " << av_err2str(ret) << "\n";
                        isRecording = false;
                        break;
                    }
                    av_packet_unref(packet);
                ...
                if(ret != AVERROR(EAGAIN) && ret != AVERROR_EOF) {
                    std::cerr << "Video encoding error: " << av_err2str(ret) << "\n";
                }
            }
            av_packet_free(&packet);
            av_frame_free(&frame);
...
//------------------------------------------------------------------------------
// Reconnection loop: Automatically reconnect if stream ends unexpectedly.
//------------------------------------------------------------------------------
